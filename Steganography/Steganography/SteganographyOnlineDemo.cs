...
namespace Steganography
{
    ...
    public static class SteganographyOnlineDemo
    {
        private static double lastPercentageDone;

        public static void Encode(string plainText, string sourceImagePath, string destinationImagePath)
        {
            // Read the image as a bitmap image
            Bitmap newbmp = new Bitmap(Image.FromFile(sourceImagePath));

            newbmp = EmbedText(plainText, newbmp);
        ...
        {
            int totalNumberOfPixels = bmp.Height * bmp.Width;
            int currentPixel = 1;
            ...
            for (int i = 0; i < bmp.Height; i++)
            {
                ...
                {
                    PrintPercentageDone(currentPixel, totalNumberOfPixels);
        ...
        public static void Decode(string sourceImagePath)
        {
            // Read the image as a bitmap image
            Bitmap newbmp = new Bitmap(Image.FromFile(sourceImagePath));

            string cypherText = ExtractText(newbmp);

            Console.WriteLine("Hidden Text Is - ");
        ...
        /// <summary>
        /// Print the percentage done of the conversion
        /// </summary>
        /// <param name="currentPixel"></param>
        /// <param name="totalNumberOfPixels"></param>
